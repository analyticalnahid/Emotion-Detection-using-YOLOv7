...
def detect(save_img=False):
    source, weights, save_txt, show_conf, imgsz, trace = opt.source, opt.weights, opt.save_txt, not opt.hide_conf, opt.img_size, not opt.no_trace
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        plot_one_box(xyxy, im0, label=label, color=colour, line_thickness=1)        
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='weights/yolo.pt', help='model.pt path(s)')
    ...
    with torch.no_grad():
        if opt.update:  # update all models (to fix SourceChangeWarning)
            for opt.weights in ['weights/yolo.pt']:
                detect(opt=opt)
